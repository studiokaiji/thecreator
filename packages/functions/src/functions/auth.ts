...
export const getNonceToSign = https.onRequest((request, response) =>
...
export const verifySignedMessage = https.onRequest((request, response) =>
  cors(request, response, async () => {
    try {
      if (request.method !== 'POST') {
        return response.sendStatus(403);
      ...
      if (userDoc.exists) {
        ...
        if (recoveredAddress === address) {
          ...
          const firebaseToken = auth.createCustomToken(address);
          // Return the token
